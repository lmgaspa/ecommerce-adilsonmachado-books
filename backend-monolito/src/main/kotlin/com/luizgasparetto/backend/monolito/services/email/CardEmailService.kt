...
class CardEmailService(
    ...
    fun sendCardClientEmail(order: Order) {
        sendEmail(
            to = order.email,
            subject = "✅ Cartão aprovado (#${order.id}) — Adylson Machado - E-Commerce",
    ...
    fun sendCardAuthorEmail(order: Order) {
        sendEmail(
            to = authorEmail,
            subject = "📦 Novo pedido pago (cartão) (#${order.id}) — Adylson Machado - E-Commerce",
    ...
    fun sendClientCardDeclined(order: Order) {
        sendEmail(
            to = order.email,
            subject = "❌ Cartão não aprovado (#${order.id}) — Adylson Machado - E-Commerce",
    ...
    fun sendAuthorCardDeclined(order: Order) {
        sendEmail(
            to = authorEmail,
            subject = "⚠️ Pedido recusado no cartão (#${order.id}) — Adylson Machado - E-Commerce",
    ...
    private fun buildHtmlMessage(order: Order, isAuthor: Boolean, declined: Boolean): String {
        ...
        val contactBlock = if (!isAuthor) """
            <p style="margin:16px 0 0;color:#555">
              Em caso de dúvida, fale com a <strong>Adylson Machado - E-CommerceLar</strong><br>
        ...
        <body style="font-family:Arial,Helvetica,sans-serif;background:#f6f7f9;padding:24px">
          <div style="max-width:640px;margin:0 auto;background:#fff;border:1px solid #eee;border-radius:12px;overflow:hidden">
            <div style="background:#0b0b0c;color:#fff;padding:16px 20px;display:flex;align-items:center;gap:10px">
              ...
              <strong style="font-size:16px">Adylson Machado - E-Commerce</strong>
            </div>
            ...
            <div style="background:#fafafa;color:#888;padding:12px 20px;text-align:center;font-size:12px">
              © ${java.time.Year.now()} Adylson Machado - E-Commerce · Todos os direitos direitos Reservados· ✉️ <a href="mailto:luhmgasparetto@gmail.com" style="color:#888;text-decoration:none">luhmgasparetto@gmail.com</a>
            </div>
          </div>
        </body>
        </html>
