...
class PixEmailService(
    ...
    private val log = org.slf4j.LoggerFactory.getLogger(PixEmailService::class.java)

    fun sendPixClientEmail(order: Order) {
        sendEmail(
            to = order.email,
            subject = "✅ Pagamento CONFIRMADO (#${order.id}) — Adylson Machado - E-Commerce",
    ...
    fun sendPixAuthorEmail(order: Order) {
        sendEmail(
            to = authorEmail,
            subject = "📦 Novo pedido pago (#${order.id}) — Adylson Machado - E-Commerce",
    ...
    private fun buildHtmlMessage(order: Order, isAuthor: Boolean): String {
        ...
        val contactBlock = if (!isAuthor) """
            <p style="margin:16px 0 0;color:#555">
              Em caso de dúvida, fale com a <strong>Adylson Machado - E-Commerce</strong><br>
        ...
        <body style="font-family:Arial,Helvetica,sans-serif;background:#f6f7f9;padding:24px">
          <div style="max-width:640px;margin:0 auto;background:#fff;border:1px solid #eee;border-radius:12px;overflow:hidden">
            ...
            <div style="background:#fafafa;color:#888;padding:12px 20px;text-align:center;font-size:12px">
              © ${java.time.Year.now()} Adylson Machado - E-Commerce · Todos os direitos Reservados· ✉️ <a href="mailto:luhmgasparetto@gmail.com" style="color:#888;text-decoration:none">luhmgasparetto@gmail.com</a>
            </div>
          </div>
        </body>
        </html>
